...
class ISBuilder(Builder):
    ...
    def __init__(
        ...
        super().__init__(params, {"material": {self.IS: material_name}, **build_config})
        self.ref_fbe = ref_fbe

    def radial_build_inboard_shield(self) -> float:
        radial_build_list = [
            self.params.r_cs_in.value,
            self.params.tk_cs.value,
            self.params.tk_tf_inboard.value,
            self.params.g_ts_tf.value,
            self.params.tk_ts.value,
            self.params.g_vv_ts.value,
            self.params.tk_vv_in.value,
            self.params.tk_sh_in.value,
        ]
        return np.sum(radial_build_list)
    ...
    def build(self) -> Component:
        ...
        shield_top_cap_wire = make_polygon(
            [(shield_wire_outer.start_point(), shield_wire_inner.start_point())],
            label="Shield top cap wire",
        )
        shield_bottom_cap_wire = make_polygon(
            [(shield_wire_outer.end_point(), shield_wire_inner.end_point())],
            label="Shield bottom cap wire",
        )
        shield_loop = BluemiraWire(
            [
                shield_wire_outer,
                shield_top_cap_wire,
                shield_wire_inner,
                shield_bottom_cap_wire,
            ],
            label="Shield loop",
        )
        shield_face = BluemiraFace(shield_loop, label="Shield face")
        shield_xyz = revolve_shape(
            shield_face,
            base=(0, 0, 0),
            direction=(0, 0, 1),
            degree=sector_degree * n_sectors,
            label="shield revolved",
        )
